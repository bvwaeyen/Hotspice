...
class Sweep(ABC):
    ...
    def plot(self, summary_file, param_x=None, param_y=None, transform_x=None, transform_y=None, name_x=None, name_y=None, title=None, save=True, plot=True, metrics_dict=None):
        ''' Generic function to plot the results of a sweep in a general way.
            @param summary_file [str]: the path to a file as generated by Sweep.load_results().
            @param param_x, param_y [str] (None): the column names of the sweeped parameters
                to be put on the X- and Y-axis, respectively.
            # TODO: Check if this function works for 1D sweeps, or higher than 3D (to select 2 dims then)
        ...
        if metrics_dict is None: # metrics is a dict of name:function(df) pairs that return (combinations of) df columns
        ...
        init_fonts()
        n_plots = len(metrics)
        fig = plt.figure(figsize=(3.3*n, 4))
        ...
        for i, name in enumerate(metrics_dict.keys()):
            ...
            if is_2D:
                ...
                ax.set_title(name)
                im = ax.pcolormesh(X, Y, metrics[i], cmap=cmap) # OPT: can use vmin and vmax, but not without a Metric() class, which I think would lead us a bit too far once again
