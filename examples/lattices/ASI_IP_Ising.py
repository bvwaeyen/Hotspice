...
t = time.perf_counter()
mm = hotspice.ASI.IP_Ising(2e-6, n, T=T, pattern='uniform', energies=[hotspice.DipolarEnergy()], PBC=True)
...
def animate_temp_rise(mm: hotspice.Magnets, animate=1, speed=100, T_step=0.05, T_max=800):
    """ Shows an animation of increasing the temperature gradually from 0 to `T_max`, which could reveal
        ...
        @param speed [int] (100): How many switches are simulated between each frame.
    """
    mm.initialize_m('AFM')
    mm.history.clear()
    AFM_ness = []
...
if __name__ == "__main__":
    ...
    # animate_temp_rise(mm, animate=3, speed=100, T_step=0.01)
