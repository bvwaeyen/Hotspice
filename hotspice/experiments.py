...
class Sweep(ABC): # TODO: add a method to finish an unfinished sweep, by specifying an output directory to be completed
    ...
    def plot(self, summary_files, param_x=None, param_y=None, unit_x=None, unit_y=None, transform_x=None, transform_y=None, name_x=None, name_y=None, logarithmic_vars: list[str] = None, title=None, save=True, plot=True, metrics: list[str] = None):
...
class TaskAgnosticExperiment(Experiment): # TODO: add a plot method to this class that plots the spatial metrics, and leave the total (non-local) metrics exposed for the Sweep.plot().
    ...
    def NL_local(self, k: int = 10, test_fraction: float = 1/4, verbose: bool = False, **kwargs) -> xp.ndarray:
        ...
        for j in range(self.n_out): # Train an estimator for the j-th output node
            ...
            results = sm.OLS(Y, X, missing='drop').fit() # missing='drop' ignores samples with NaN (here: the first k-1 samples)
    ...
    # TODO: make sure this Parity Check absolutely works as intended, because Thermal OOP_Square gives PC≈2.0 for MC≈0.1 which seems odd.
